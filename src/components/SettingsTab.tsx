...
import { ShieldCheck, Database, RefreshCw, Check, Copy, Code, HelpCircle, AlertCircle, AlertTriangle } from "lucide-react";
import { supabase } from "../lib/supabase";
import { getLegacyQrMode, setLegacyQrMode } from "../lib/studentService";
...
export const SettingsTab: React.FC<{ onResetCompleted?: () => void }> = ({ onResetCompleted }) => {
  ...
  const loadLegacyMode = async () => {
    try {
      const { legacyMode: enabled, notInitialized } = await getLegacyQrMode();
      setLegacyMode(enabled);
      setDbInitialized(!notInitialized);
      localStorage.setItem("legacy_qr_mode", String(enabled));
    } catch (e) {
      console.error("Failed to load legacy mode setting:", e);
...
-- 6. CREATE SYSTEM CONFIGURATION SETTINGS TABLE
...
-- CONFIGURE SELECT POLICY FOR ALL (Guards and frontend need to read this state)
          ...
          <div className="p-6 space-y-4">
            <div className="flex items-center justify-between p-4 bg-slate-50 border border-slate-200 rounded-xl">
              <div className="space-y-1 pr-4">
                ...
                {dbInitialized === false && (
                  <span className="inline-flex items-center gap-1 text-[9px] font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded border border-amber-200 mt-1">
                    <AlertTriangle className="w-3 h-3 text-amber-500" />
                    Local Mode Only (Database table missing)
                  </span>
                )}
                {dbInitialized === true && (
                  <span className="inline-flex items-center gap-1 text-[9px] font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200 mt-1">
                    <Check className="w-3 h-3 text-emerald-500" />
                    Cloud Persisted (Synced across all terminals)
                  </span>
                )}
              </div>
              <button
                ...
                onClick={async () => {
                  ...
                  if (dbInitialized === false) {
                    ...
                    const res = await setLegacyQrMode(newVal, adminPassword);
                    if (res.success) {
                      setLegacyMode(newVal);
                      localStorage.setItem("legacy_qr_mode", String(newVal));
                    } else {
                      if (res.notInitialized) {
                        ...
                        setLegacyMode(newVal);
                        localStorage.setItem("legacy_qr_mode", String(newVal));
                        alert("Settings table not found in database. Temporarily fell back to local browser storage. Please run the SQL setup script.");
                      } else {
                        alert(res.error || "Failed to update legacy QR mode setting on server.");
                      }
                    }
                  }
                  setSavingSettings(false);
                }}
                className={`w-12 h-6 flex items-center rounded-full p-1 cursor-pointer transition-colors duration-300 focus:outline-hidden disabled:opacity-50 ${
            ...
            {dbInitialized === false && (
              <div className="p-3 bg-rose-50 border border-rose-200 text-rose-800 text-[10px] rounded-lg flex items-start gap-2">
                <AlertCircle className="w-4 h-4 text-rose-600 flex-shrink-0 mt-0.5" />
                <span>
                  <strong>Database Table Missing:</strong> The <code>system_settings</code> table is not initialized in your Supabase database. Toggling this mode will only apply to this browser session and won't sync to the guard terminal. Copy and execute the SQL setup script on the right in the Supabase SQL editor.
                </span>
              </div>
            )}
